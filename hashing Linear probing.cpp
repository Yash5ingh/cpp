...
void Insert(int ary[], int hFn, int Size)
{
    ...
    { // INT_MIN and INT_MAX indicates that cell is empty. So if cell is empty loop will break and goto bottom of the loop to insert element
        if (ary[pos] == INT_MAX)
            break;
        // algorithm for with replacement
        /*else //if position is not empty
        //take ary[pos] element in some variable, eg. value
        //assign element to ary[pos]
        //find out next index 'pos' and check whether it is empty or not
        //if it is empty then put value ary[pos]
        //else call same process of finding next index and checking it is empty or not*/
        pos = (pos + 1) % hFn;
        n++;
        if (n == Size)
